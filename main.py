...
class BookingApp:
    ...
    def book_appointment(self):
        ...
        if not name or not date or not time:
            ...
            self.appointments.append(Appointment(name, date.strftime('%Y-%m-%d'), time))
    ...
    def update_appointment(self):
        ...
        if not new_time:
            ...
            self.appointments[int(selected[0]) - 1] = Appointment(selected_name, selected_date, new_time)
    ...
    def load_appointments(self):
        if os.path.isfile('appointments.csv'):
            with open('appointments.csv', newline='') as f:
                reader = csv.reader(f)
                self.appointments = [Appointment(row[0], row[1], row[2]) for row in reader]
